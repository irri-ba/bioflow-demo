      ...
      if(!is.null(data()$data$pedigree)){
        ...
        if( length(which(colnames(myYears) %in% "yearOfOrigin")) > 0){
          ...
          if(input$methodRgg == "mackay"){
            ...
            updateSelectInput(session, "effectTypeToUse", choices = traitsRgg, selected =traitsRgg[1] )
          } else{
            updateSelectInput(session, "effectTypeToUse", choices="", selected="")
            traitsRgg1 <- unique(dtRgg$entryType)
            updateSelectInput(session, "entryTypeToUse", choices = traitsRgg1, selected =traitsRgg1[1] )
          }
        }
      }
    })
    ## entry type
    observeEvent(c(data(), input$methodRgg, input$version2Rgg, input$trait2Rgg, input$yearsToUse, input$environmentToUse, input$effectTypeToUse), {
      req(data())
      req(input$methodRgg)
      req(input$version2Rgg)
      req(input$trait2Rgg)
      ...
      if(!is.null(data()$data$pedigree)){
        ...
        if( length(which(colnames(myYears) %in% "yearOfOrigin")) > 0){
          ...
          dtRgg <- dtRgg[which(dtRgg$environment %in% input$environmentToUse),]
          if(input$methodRgg == "mackay"){
            dtRgg <- dtRgg[which(dtRgg$effectType %in% input$effectTypeToUse),]
          }
