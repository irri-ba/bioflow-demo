...
if (is.null(result)){
  ...
  if(params$toDownload == TRUE){ # we will download the document
  ...
  if(params$toDownload == TRUE){ # we will download the document
    ...
    inputPanel(
      selectInput(ns("boxplotMtaPredsType"),
                  label = "Effect type",
                  choices = if(sum(stringr::str_detect(effectTypes, "_designation")) > 1){
                    c("environment_designation", "designation")
                  } else{effectTypes[effectTypes %in% c("environment_designation", "designation")]}
      ),
      column(width=12),
...
if (is.null(result)){
  ...
  if(sum(stringr::str_detect(effectTypes, "_designation")) > 1){
    effectTypes_BEMD <- c("environment_designation", "designation")
  } else{
    effectTypes_BEMD <- effectTypes[effectTypes %in% c("environment_designation", "designation")]
  }
  
  if(params$toDownload == TRUE){ # we will download the document
    
    cat(paste0("###  {.tabset .tabset-pills}    \n\n  "))
    for(iType in effectTypes_BEMD){
      cat("\n\n####", iType, "{.tabset .tabset-pills}       \n\n")
      for(iTrait in traits){
        cat("\n\n#####", iTrait, "{.tabset .tabset-pills}       \n\n")
        if(sum(stringr::str_detect(effectTypes, "_designation")) > 1){
          if(iType == "environment_designation"){
            level1 <- predSplitSplit[effectTypes[which(stringr::str_detect(effectTypes, "_designation"))]]
    ...
    plotly::renderPlotly({
      if(sum(stringr::str_detect(effectTypes, "_designation")) > 1){
        if(input$boxplotMtaPredsType == "environment_designation"){
          level1 <- predSplitSplit[effectTypes[which(stringr::str_detect(effectTypes, "_designation"))]]
      ...
      if(!is.null(temp.pred.box)){
        if(nrow(temp.pred.box) > 0){
          ...
          if(input$boxplotMtaPredsType == "designation"){
...
if (is.null(result)){
  ...
  if(params$toDownload == TRUE){ # we will download the document
    ...
    } else if(sum(stringr::str_detect(effectTypes, "_designation")) == 1){
      
      if(effectTypes[which(stringr::str_detect(effectTypes, "_designation"))] == "environment_designation"){
        inputPanel(
          selectInput(ns("traitPredictionsCorrelation"),
                      label = "Trait",
                      choices = traits
  ...
  if(sum(stringr::str_detect(effectTypes, "_designation")) > 1){
    if(params$toDownload == TRUE){ # we will download the document
      ...
      for(iTrait2 in traits){
        ...
        level1 <- predSplitSplitWide[effectTypes[which(stringr::str_detect(effectTypes, "_designation"))]]
        level2 <- lapply(level1, function(x){x[iTrait2]})
        level3 <- NULL
        
        for(i in 1:length(level2)){
          level2[[i]][[1]]$designation <- gsub(paste0(colnames(level2[[i]][[1]]),":"),"",row.names(level2[[i]][[1]]))
          if(i == 1){
            level3 <- level2[[i]][[1]]
          } else{
    ...
    if(effectTypes[which(stringr::str_detect(effectTypes, "_designation"))] == "environment_designation"){
      ...
      if(params$toDownload == TRUE){ # we will download the document
        ...
        for(iTrait2 in traits){
          ...
          wide2 <- predSplitSplitWide[["environment_designation"]][[iTrait2]] 
        ...
        plotly::renderPlotly({
          ...
          level1 <- predSplitSplitWide["environment_designation"]
          level2 <- lapply(level1, function(x){x[input$traitPredictionsCorrelation]})
          ...
          if(length(level3)>0){
            N <- sort(unlist(lapply(level3,length)), decreasing = TRUE)
            wide2 <- do.call(cbind,level3[ names(N)[which(N == N[1])] ])
            colnames(wide2) <- gsub(paste0("environment_designation.",input$traitPredictionsCorrelation,"."),"",colnames(wide2))
            corr <- round(stats::cor(wide2, use="pairwise.complete.obs"),2)
            if(nrow(corr) > 1){
              if( length(which(is.na(corr))) == 0){
                ord <- hclust( dist(corr, method = "euclidean"), method = "ward.D" )$order
      ...
      for(iTrait3 in traits){
        ...
        if(length(level3)>0){
          ...
          for(i in 1:ncol(pred.mta.biplot)){
            rownames(pred.mta.biplot) <- gsub(paste0(colnames(pred.mta.biplot)[i],":"),"",rownames(pred.mta.biplot))
          }
      ...
      if(params$toDownload == TRUE){ # we will download the document
        ...
        for(iTrait3 in traits){
          ...
          if(length(level3)>0){
            N <- sort(unlist(lapply(level3,length)), decreasing = TRUE)
            pred.mta.biplot <- do.call(cbind,level3[names(N)[which(N == N[1])]])
            colnames(pred.mta.biplot) <- gsub(paste0("environment_designation.",iTrait3,"."),"",colnames(pred.mta.biplot)) 
            rownames(pred.mta.biplot) <- rownames(level2[[ which(N == N[1])[1] ]][[1]] )
            for(i in 1:ncol(pred.mta.biplot)){
              rownames(pred.mta.biplot) <- gsub(paste0(colnames(pred.mta.biplot)[i],":"),"",rownames(pred.mta.biplot))
            }
        ...
        plotly::renderPlotly({
          ...
          if(length(level3) > 0){
            ...
            pred.mta.biplot <- do.call(cbind,level3[names(N)[which(N == N[1])]])
            rownames(pred.mta.biplot) <- rownames(level2[[ which(N == N[1])[1] ]][[1]] )
            colnames(pred.mta.biplot) <- gsub(paste0("environment_designation.",input$traitBiplot,"."),"",colnames(pred.mta.biplot)) 
            rownames(pred.mta.biplot) <- rownames(level2[[ which(N == N[1])[1] ]][[1]] )
            for(i in 1:ncol(pred.mta.biplot)){
              rownames(pred.mta.biplot) <- gsub(paste0(colnames(pred.mta.biplot)[i],":"),"",rownames(pred.mta.biplot))
            }
...
if (is.null(result)){
  ...
  if(params$toDownload == TRUE){ # we will download the document
    ...
    effectTypesMT <- if(sum(stringr::str_detect(effectTypes, "_designation")) > 1){
      c("environment_designation", "designation")
    } else{effectTypes[effectTypes %in% c("environment_designation", "designation")]}
    
    for(iType4 in effectTypesMT){
      cat("\n\n####", iType4, "{.tabset .tabset-pills}       \n\n")
      for(iTrait4 in traits){
        cat("\n\n#####", iTrait4, "{.tabset .tabset-pills}       \n\n")
        if(iType4 %in% c("environment", "designation", "(Intercept)")){
        ...
        for(iEnv4 in envirTypes2){
          ...
          if(sum(stringr::str_detect(effectTypes, "_designation")) > 1){
            if(iType4 == "environment_designation"){
              level1 <- predSplitSplit[effectTypes[which(stringr::str_detect(effectTypes, "_designation"))]]
    ...
    plotly::renderPlotly({
      ...
      if(length(input$scatterMtaPredsType) > 0){
        ...
        if(sum(stringr::str_detect(effectTypes, "_designation")) > 1){
          if(input$scatterMtaPredsType == "environment_designation"){
            level1 <- predSplitSplit[effectTypes[which(stringr::str_detect(effectTypes, "_designation"))]]
